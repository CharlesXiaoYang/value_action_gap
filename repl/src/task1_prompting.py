class Task1Prompting:
    ...
    def context_prompt(self, country: str, scenario: str, index: str = "claim") -> str:
        if index == "claim":
    ...
    def value_prompt(self, index: str = "claim") -> str:
        lines = []
        if index == "claim":
            for value_name, value_statement in self.value_claims.items():
                lines.append(f"{value_name}: {value_statement}.")
        else:
            for value_name, value_statement in self.value_portraits.items():
                lines.append(f"{value_name}: she/he values {value_name} and {value_statement}.")
        return "\n".join(lines) + "\n"

    def option_prompt(self, index: str = "claim") -> str:
        if index == "claim":
    ...
    def requirement_prompt(self, index: str = "chat") -> str:
        if index == "chat":
    ...
    def generate_prompt(self, country: str, scenario: str, index: int = 0) -> str:
        if index == 0:
